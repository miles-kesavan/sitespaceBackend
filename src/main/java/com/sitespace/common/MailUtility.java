...
public class MailUtility {
		...
		public static void sendMailTalentchek(Email email, String path) throws Exception {
			...
			try {
				...
				//String[] to = {"ravi@paragondynamics.in"};  //sgopes@gmaill.com","buvaneshwari@paragondynamics.inss
				//String[] bcc = {"raghavdhuruv.07@gmail.com"}; 
				email.setToEmailAddress(email.getToEmailAddress());
				email.setCcEmailAddress(email.getCcEmailAddress());
				email.setBccEmailAddress(email.getBccEmailAddress());
				Transport bus = session.getTransport("smtp");	
	...
	//api goes here			
       ...
       public static void sendBulkMail(Email email, String path) throws Exception {
...
// api key
	...
	public static void sendMailWithAttachmentVisitor(Email email,MultipartFile attachment) throws MessagingException{
	...
	// api key
		...
		//if (!"".equals(sFilePath)) {
		messageBodyPart = new MimeBodyPart();
		DataSource source = new FileDataSource(convertMultipartFileToFile(attachment));
		messageBodyPart.setDataHandler(new DataHandler(source));
		messageBodyPart.setFileName(source.getName());
		multipart.addBodyPart(messageBodyPart);
